...
using (UcsFacade ucs = UcsFacade.connectToUcs("localhost"))
{
    ...
    var onlineVehicles = ucs.List<Vehicle>()
        // Keep only online vehicles.
        .Where(v => ucs.IsOnline(v));
    ...
    // Some vehicles (emulator, for example) do not support 'takeoff_command'.
    // They take off during arming.

    if (ucs.IsTakeoffSupported(selectedVehicle))
    {
