...
namespace MapControl
{
    ...
    public partial class MapBase : MapPanel
    {
        ...
        public static readonly DependencyProperty IsMouseZoomEnabledProperty = DependencyProperty.Register(
            "IsMouseZoomEnabled", typeof(bool), typeof(MapBase),
            new PropertyMetadata(false, (o, e) => ((MapBase)o).IsMouseZoomEnabledPropertyChanged((bool)e.NewValue)));

        public static readonly DependencyProperty IsMouseMoveEnabledProperty = DependencyProperty.Register(
            "IsMouseMoveEnabled", typeof(bool), typeof(MapBase),
            new PropertyMetadata(false, (o, e) => ((MapBase)o).IsMouseMoveEnabledPropertyChanged((bool)e.NewValue)));        
        ...
        public double GetMapZoomScale(Location location, double scale)
        {
            double zoomLevel = Math.Log(scale * (TileSource.MetersPerDegree * 360d) / (mapTransform.RelativeScale(location) * (double)TileSource.TileSize)) / Math.Log(2d);
            return zoomLevel;     
        }
        ...
        public void TransformMapZoom(double scale)
        {
            if (scale != 1d)
            {
                var zoomLevel = Math.Min(Math.Max(ZoomLevel + Math.Log(scale, 2d), MinZoomLevel), MaxZoomLevel);
        ...
        /// </summary>
        public void ZoomMap(Point origin, double zoomLevel, bool zoomOrigin = true)
        {
        ...
        private void IsMouseZoomEnabledPropertyChanged(bool enabled)
        {
            IsMouseZoomEnabled = enabled;
        }

        private void IsMouseMoveEnabledPropertyChanged(bool enabled)
        {
            IsMouseMoveEnabled = enabled;
        }
        ...
        {
            get { return (bool)GetValue(IsMouseZoomEnabledProperty); }
            set { SetValue(IsMouseZoomEnabledProperty, value); }
        }
